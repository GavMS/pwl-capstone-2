...
.back-btn {
    ...
    background: #f0f2f5; border-radius: .5rem;
    color: #7b809a; text-decoration: none;
...
.field-label {
    ...
    color: #344767; margin-bottom: .5rem;
}
...
.pwd-toggle:hover { color: #7928ca; }
/* Hint text */
.field-hint {
    margin-top: .4rem;
    font-size: .775rem; color: #adb5bd;
}
...
<div class="flex flex-wrap -mx-3">
            ...
            <div style="display:flex; align-items:center; gap:1rem;">
                ...
                <div>
                    <h4 class="form-page-title">
                        @if(isset($editUser))
                            <i class="fas fa-user-edit" style="color:#7928ca; font-size:1rem; margin-right:.4rem;"></i>Edit Pengguna
                        @else
                            <i class="fas fa-user-plus" style="color:#7928ca; font-size:1rem; margin-right:.4rem;"></i>Tambah Pengguna Baru
                        @endif
                    </h4>
    ...
    {{-- ─── Smart Edit Warning ───────────────────── --}}
    @if(isset($editUser) && count($deps) > 0)
    <div class="w-full px-3 mb-4">
        <div class="warn-banner">
            <i class="fas fa-exclamation-triangle"></i>
            <div>
                <p class="warn-banner-title">Perhatian: User Ini Memiliki Data Terkait</p>
                <p class="warn-banner-sub">Mengubah role pengguna dapat mempengaruhi data berikut:</p>
                <ul class="warn-dep-list">
                    @foreach($deps as $dep)
                        <li>{{ $dep['label'] ?? '-' }} ({{ $dep['count'] ?? 0 }} data)</li>
                    @endforeach
                </ul>
            </div>
        </div>
    ...
    <div class="w-full px-3">
        <div class="form-card">
            ...
            <div class="form-body">
                ...
                <p class="section-label"><i class="fas fa-id-card"></i> Informasi Akun</p>
                    ...
                    <div class="field-group">
                        ...
                        <div class="input-wrap">
                            ...
                            <input type="email" id="email" name="email" class="form-input"
                                   ...
                                   {{ isset($editUser) ? 'readonly style=background-color:#f8f9fa;cursor:not-allowed;' : '' }} />
                        </div>
                        @if(isset($editUser))
                            <p class="field-hint"><i class="fas fa-info-circle"></i> Email digunakan untuk login dan tidak dapat diubah.</p>
                        @endif
                    </div>

                    {{-- Role --}}
                    <div class="field-group">
                        <label class="field-label" for="role_id">
                ...
                {{-- ── Section: Keamanan ──── --}}
                <p class="section-label"><i class="fas fa-lock"></i> Keamanan</p>

                <div class="field-group">
                    <label class="field-label" for="password">
                        Password
                        @if(isset($editUser))
                            <span class="opt">(Kosongkan jika tidak ingin mengubah)</span>
                        @else
                            <span class="req">*</span>
                        @endif
                    </label>
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" class="btn-submit-form">
                        <i class="fas fa-{{ isset($editUser) ? 'save' : 'user-plus' }}" style="font-size:.75rem;"></i>
                        {{ isset($editUser) ? 'Simpan Perubahan' : 'Tambah User' }}
                    </button>
                </div>
